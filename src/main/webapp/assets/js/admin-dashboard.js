...
function switchTab(tab) {
  ...
  document.getElementById('tab-overview').classList.toggle('d-none', tab !== 'overview');
  document.getElementById('tab-stats').classList.toggle('d-none', tab !== 'stats');

  const url = new URL(window.location);
  url.searchParams.set('tab', tab);
  window.history.replaceState({}, '', url);

  // On synchronise le hidden input du formulaire de filtre de dates
  const tabInput = document.querySelector('#dateFilterForm input[name="tab"]');
